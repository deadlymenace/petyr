/**
 * LangSmith Evaluation Runner for Petyr
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps Petyr agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `petyr-finance-eval-sample-${sampleSize}-${Date.now()}`
      : 'petyr-finance-eval';
    ...
    const experimentName = `petyr-eval-${Date.now().toString(36)}`;
